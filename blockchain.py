# -*- coding: utf-8 -*-
"""
Created on Wed Jan 15 19:29:06 2020
...
class Blockchain:
    ...
    Attributes:
        ...
        :public_key: Public key of connected node (which runs the blockchain).
    """

    def __init__(self, public_key, node_id):
        # Create first block as block object
    ...
    def get_balance(self, sender=None):
        """ Calculate and return the balance of the hosting node """
        if sender == None:
            ...
            participant = sender
        # Fetch a list of all coins sent from a given person (empty lists are)
        # This fetches the amounts sent in transactions already included in the blockchain
        ...
        amount_sent = reduce(lambda tx_sum, tx_amt: tx_sum + sum(float(i)
                                                                 for i in tx_amt) if len(tx_amt) > 0 else tx_sum + 0, tx_sender, 0)
        # This fetches received coin amounts of transactions that were already included in blocks of the blockchain
        # We ignore open transactions here because you shouldn't be able to spend coins before the transaction was confirmed + included in a block
        tx_recipient = [[tx.amount for tx in block.transactions if tx.recipient ==
                         participant] for block in self.__chain]
        amount_received = reduce(lambda tx_sum, tx_amt: tx_sum + sum(float(i)
                                                                     for i in tx_amt) if len(tx_amt) > 0 else tx_sum + 0, tx_recipient, 0)
    ...
    # One requires a one (transaction amount) and one optional one (last transaction)
    # The second one is optional because it has a default value => [1]
    # =============================================================================
    ...
    def add_transaction(self, recipient, sender, signature, amount=1.0, is_receiving=False):
        """ Append a new value as well as the last blockchain value to the blockchain.
            ...
            Arguments:
                :sender: Sender in transaction.
                :recipient: Recipient in transaction.
                :amount: Transaction amount in $$. Default = 1.0.
        """
        # if self.public_key == None:
        #    return False
    ...
    def mine_block(self):
        if self.public_key == None:
            return None
        # Fetch the current last block of the chain
        last_block = self.__chain[-1]
        # Hash the last block for comparison with stored hash
